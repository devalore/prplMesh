...
uint16_t& cACTION_APMANAGER_CLIENT_DISALLOW_REQUEST::validity_period() {
    return (uint16_t&)(*m_validity_period);
}
...
void cACTION_APMANAGER_CLIENT_DISALLOW_REQUEST::class_swap()
{
    ...
    m_mac->struct_swap();
    m_bssid->struct_swap();
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_validity_period));
}
...
size_t cACTION_APMANAGER_CLIENT_DISALLOW_REQUEST::get_initial_size()
{
    ...
    class_size += sizeof(uint16_t); // validity_period
    return class_size;
}
...
bool cACTION_APMANAGER_CLIENT_DISALLOW_REQUEST::init()
{
    ...
    m_validity_period = (uint16_t*)m_buff_ptr__;
