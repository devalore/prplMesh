...
bool monitor_thread::handle_cmdu(Socket *sd, ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    case beerocks_message::ACTION_MONITOR_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY: {
        ...
        if (!request) {
            LOG(ERROR) <<
                "addClass cACTION_MONITOR_CLIENT_ASSOCIATED_STA_LINK_METRIC_QUERY failed";
            return false;
        }
...
bool monitor_thread::hal_event_handler(bwl::base_wlan_hal::hal_event_ptr_t event_ptr)
{
    ...
    case Event::RRM_Associated_STA_Link_Metrics_Response: {
        ...
        response->bss_num() = metrics_result->bss_num;
        
        if (!response->alloc_per_bss_associated_sta_link_metrics(response->bss_num())) {
