                                               ...
                                               std::shared_ptr<beerocks_header> beerocks_header)
{
    ...
    case beerocks_message::ACTION_MONITOR_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE: {
        ...
        if (!response_in) {
            LOG(ERROR) << "addClass cACTION_MONITOR_CLIENT_ASSOCIATED_STA_LINK_METRIC_RESPONSE failed";
            return false;
        }

        if (!cmdu_tx.create(0, ieee1905_1::eMessageType::ASSOCIATED_STA_LINK_METRICS_RESPONSE_MESSAGE)) {
...
bool slave_thread::handle_associated_sta_link_metrics_query(Socket *sd, ieee1905_1::CmduMessageRx &cmdu_rx)
{
    const auto mid = cmdu_rx.getMessageId();
    LOG(DEBUG) << "Received ASSOCIATED_STA_LINK_METRICS_QUERY_MESSAGE, mid=" << std::hex << int(mid);

    auto mac_tlv = cmdu_rx.getClass<ieee1905_1::tlvStaMacAddressType>();
    if (!mac_tlv) {
        LOG(ERROR) << "ASSOCIATED_STA_LINK_METRICS_QUERY_MESSAGE does not have tlvStaMacAddressType";
        return false;
    }
